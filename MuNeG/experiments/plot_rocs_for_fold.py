...
def plot(figure, fold, function="", qty=0):
    ...
    plt.title('Receiver operating characteristic for ' + str(int(float(fold))) + ' folds', fontsize="25")
    legend = plt.legend(loc="lower right", fontsize="17", frameon=True)
    frame = legend.get_frame()
    frame.set_facecolor('white')
    frame.set_edgecolor('black')
    # plt.draw()
    file_name = function.replace(":", "") if function <> "" else "fold_20"
    if file_name <> "fold_20":
        file_name += "_" + str(qty)
...
while True:
    ...
    if weight >= 30:
        ...
        fprs_file = open('..\\results\\real\\roc2\\mean_' + 'fprs' + '_' + function_for_file_name + '_' + str(weight) + '_' + fold + '.txt', 'rb')
        mean_fprs = pickle.load(fprs_file)
        tprs_file = open('..\\results\\real\\roc2\\mean_' + 'tprs' + '_' + function_for_file_name + '_' + str(weight) + '_' + fold + '.txt', 'rb')
