...
class DecisionFusion(object):
    ...
    LAYERS_WEIGHTS = []
    REAL_LAYERS_WEIGHTS = [i for i in xrange(1, 9)]
    LAYERS_NAME = []
    ...
    def processExperiment(self, class_label):
        self.readRealData(class_label)
        self.preprocessing()
        self.flatLBP()
        self.multiLayerLBP()
        # self.rwpLBP()
    ...
    Prepare data
    '''      
    def readRealData(self, class_label):
        reader = SocialEvolutionReader()
        reader.read(class_label)
            ...
            for id in xrange(0, len(self.realLabels)):
                ...
                # if True:
                    if folds > 0:
                        ...
                        new_reduction_scores.append([elem/(folds-1) for elem in self.realFlatScores[id]])
