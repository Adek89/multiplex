...
def execute_experiment(method, folds, class_label):
    gc.collect()
    df = DecisionFusion(method, folds)
    return df.processExperiment(class_label)
...
def plot(figure, qty=0, file_name = 'global'):
    ...
    figure.savefig('/lustre/scratch/apopiel/real_se/roc2/' + file_name + '.png', dpi=300)
    plt.close(figure)
...
def save_mean_rates(means, rate_type, class_label, probe, fold=None):
    target_file = open('/lustre/scratch/apopiel/real_se/means/mean_' + rate_type + '_' + (str('_'+str(fold)) if fold <> None else '') + class_label + '_' + str(probe) + '.txt', 'ab')
    pickle.dump(means, target_file)
    target_file.close()

def save_aoc_results(probe, aoc, key, class_label, fold ='', file_name = 'global'):
    with open('/lustre/scratch/apopiel/real_se/aoc/real_aoc_' + file_name + '_' + class_label + '_' + str(probe) + '.csv','ab') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([key, fold, class_label, aoc, probe])

def calculate_layer_results(class_label, probe, fprs_per_method, tprs_per_method, nrOfLayers, fold='', file_name='global'):
    auc_for_layer = {}
    for l in xrange(1, nrOfLayers+1):
        ...
        save_aoc_results(probe, roc_auc, key_for_layer, class_label, fold=fold, file_name=file_name)
    ...
    save_aoc_results(probe, max_layer_result, "max_layer", class_label, fold=fold, file_name=file_name)
    save_aoc_results(probe, min_layer_result, "min_layer", class_label, fold=fold, file_name=file_name)
    save_aoc_results(probe, avg_layer_result, "avg_layer", class_label, fold=fold, file_name=file_name)
...
if __name__ == "__main__":
        ...
        while execute:
            ...
            for fold in [2.0, 3.0, 4.0, 5.0, 10.0, 20.0]:
                ...
                fprs_per_method = {}
                tprs_per_method = {}
                fprs_per_method, tprs_per_method = execute_experiment(1, fold, class_label)
                ...
                for key in keys:
                    ...
                    save_aoc_results(probe, roc_auc, key, class_label, fold=fold, file_name='fold_'+str(fold))

                calculate_layer_results(class_label, probe, fprs_per_method, tprs_per_method, nrOfLayers, fold=fold, file_name='fold_'+str(fold))

                plot(fold_figure, file_name='fold_'+str(fold) + '_' + class_label + '_' + str(probe))
                save_mean_rates(fprs_per_method, 'fprs', class_label, probe, fold=fold)
                save_mean_rates(tprs_per_method, 'tprs', class_label, probe, fold=fold)
                for key in keys:
            ...
            for key in keys:
                ...
                save_aoc_results(probe, roc_auc, key, class_label)
            auc_for_layer = {}
            for key in xrange(1, nrOfLayers+1):
                key_for_layer = 'L' + str(key)
                ...
                save_aoc_results(probe, roc_auc, 'L'+str(key), class_label)
                auc_for_layer.update({key: roc_auc})
            layer_results = auc_for_layer.values()
            min_layer_result = min(layer_results)
            max_layer_result = max(layer_results)
            avg_layer_result = sum(layer_results) / float(len(layer_results))
            save_aoc_results(probe, max_layer_result, "max_layer", class_label)
            save_aoc_results(probe, min_layer_result, "min_layer", class_label)
            save_aoc_results(probe, avg_layer_result, "avg_layer", class_label)

            plot(figure, file_name='global_' + class_label + '_' + str(probe))
            save_mean_rates(mean_fprs, 'fprs', class_label, probe)
            save_mean_rates(mean_tprs, 'tprs', class_label, probe)
            execute = False
