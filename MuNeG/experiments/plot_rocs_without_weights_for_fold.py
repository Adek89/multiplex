...
def plot(figure, function="", fold=0):
    ...
    # plt.draw()
    file_name = function.replace(":", "") if function <> "" else "fold"
    file_name += "_" + str(fold)
        ...
        for probIn in [5, 6, 7, 8, 9]:
            for probBetween in [0.1, 0.5, 1, 2, 3, 4, 5]:
                for nrOfLayers in [2, 3, 4, 5, 6, 8, 10, 21]:
                    ...
                    mean_fprs = {}
                    mean_tprs = {}
                    fprs_file = open('/lustre/scratch/apopiel/synthetic/means/mean_' + 'fprs' + '_' + str(nodes) + '_' + str(size) + '_' + str(label) + '_' + str(probIn) + '_' + str(probBetween) + '_' + str(nrOfLayers) + '_' + str(fold) + '_11.txt', 'rb')
                    mean_fprs = pickle.load(fprs_file)
                    tprs_file = open('/lustre/scratch/apopiel/synthetic/means/mean_' + 'tprs'+ '_' + str(nodes) + '_' + str(size) + '_' + str(label) + '_' + str(probIn) + '_' + str(probBetween) + '_' + str(nrOfLayers) + '_' + str(fold) + '_11.txt', 'rb')
...
plot(figure, function="", fold=fold)
pass